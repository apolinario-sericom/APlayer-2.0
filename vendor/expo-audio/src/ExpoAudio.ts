...
/** Subscribes to the previous/next actions selected in Android lock-screen controls. */
export function addLockScreenActionListener(listener: (event: { action: 'previous' | 'next' }) => void) {
  return (AudioModule as unknown as {
    addListener: (
      eventName: string,
      callback: (event: { action: 'previous' | 'next' }) => void
    ) => { remove: () => void };
  }).addListener('lockScreenAction', listener);
}

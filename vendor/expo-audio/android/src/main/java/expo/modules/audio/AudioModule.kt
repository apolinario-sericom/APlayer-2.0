...
class AudioModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    Name("ExpoAudio")
    Events("lockScreenAction")

    OnCreate {
      audioManager = appContext.reactContext?.getSystemService(Context.AUDIO_SERVICE) as AudioManager
      AudioControlsService.setActionListener { action ->
        sendEvent("lockScreenAction", mapOf("action" to action))
      }
    }
    ...
    OnDestroy {
      AudioControlsService.setActionListener(null)
      appContext.mainQueue.launch {
        releaseAudioFocus()
        players.values.forEach {
          it.ref.stop()
